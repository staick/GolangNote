...
func main() {
	...
	//go语言在使用指针时，会使用内部的垃圾回收机制(gc)，开发人员不需要手动释放内存
	//c语言不允许返回栈上的指针，go语言可以返回栈上的指针，程序会在编译的时候就确定了变量分配位置：
	...
	*name2Ptr = "Duke"
	...
	//可以返回栈上的指针，返回一个string类型的指针，会自动判断这段代码，将city变量分配在堆上，内存逃逸
	res := testPtr()
	fmt.Println("res city:", *res)

	//空指针，在c语言中：null c++：nullptr go:nil
	if res == nil {
